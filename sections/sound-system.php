...
<!-- check for average time device is turned off in the daytime pm -->                                                                   <?php
                            ...
                            <div class="row" id="rowmbc">
                                ...
                                <div class="col-5">
                                  <br>
                                   ...
                                   <label class="boldsm">Sound System Status</label>
  
                                   
                                    <input type="checkbox" name="status" id="status" <?php appliance_status("Sound System");?> />

                                    <input type="hidden"  name="hidden_sound" id="hidden_sound" value="on" />
							   
							   <input type="hidden"  name="hidden_soundsystem" id="sound" value="Sound System" />
                                    <br />
                               <input type="submit" name="insert" id="action" class="btn btn-info" value="Process" />
                                ...
                                <div class="row">                                 
                                  ...
                                  <button class="accordion" style="text-align:center;">Sound System Prediction</button>
                                  <div class="panel">
                                  <br>
								
                                      <span style="font-weight:500;  padding-right:10px;">
                                      <?php
                                      $chistatus= chidi('Sound System');
                                      $dd = date("H:i:s");
 ...
 $(document).ready(function(){
  ...
  $('#status').change(function(){
   ...
   {
    $('#hidden_sound').val('off');
   }
  });

  $('#insert_data').on('submit', function(event){
   event.preventDefault();
   var stat =  $('#hidden_sound').val();
   
//alert(stat);
   $confirm = confirm("Are you sure you want to switch "+stat+ " the sound? ");
   if($confirm == true)
   {
	  
    var form_data = $(this).serialize();
    $.ajax({
     url:"processsound.php",
    method:"POST",
    data:form_data,
