...
<html lang="en">
 ...
 <?php include("../include/sectioncss.php") ?>
    ...
    <!-- check for average time device is turned off in the daytime pm -->                                                                   <?php
					...
					FROM appliance where appliance_name = 'Air Condition' and status='off' and time(date) between '12:00' and '24:00'");								
          ...
          <!-- check for average time device is turned on in the daytime pm -->                                                                   <?php
					...
					FROM appliance where appliance_name = 'Air Condition' and status='on' and time(date) between '12:00' and '24:00'");								
         ...
         <?php           
          $pred2 = mysqli_query($conn,"SELECT SEC_TO_TIME(AVG(TIME_TO_SEC(date))) as avgtime2 
					FROM appliance where appliance_name = 'Air Condition' and status='off' and time(date) between '01:00' and '12:00'");
          ...
          $pred4 = mysqli_query($conn,"SELECT SEC_TO_TIME(AVG(TIME_TO_SEC(date))) as avgtime2 
					FROM appliance where appliance_name = 'Air Condition' and status='on' and time(date) between '01:00' and '12:00'");
                            ...
                            <div class="row" id="rowmbc">
                                ...
                                <div class="col-5">
                                  <br>
                                   ...
                                   <label class="boldsm">Air Condition Status</label>
  
                         <input type="checkbox" name="status" id="status" 
						<?php appliance_status("Air Condition");?>/>

                         <input type="hidden"  name="hidden_ac" id="hidden_ac" value="on" />
							   
							   <input type="hidden"  name="hidden_aircondition" id="ac" value="Air Condition" />
                                    <br />
                               <input type="submit" name="insert" id="action" class="btn btn-info" value="Process" />
                                ...
                                <div class="row">                                 
                                  <div class="col-12"style="margin-left:20%; width:100%">
                                  <button class="accordion" style="text-align:center;">Air Condition Prediction</button>
                                  <div class="panel">
                                  <br>
								
                                      <span style="font-weight:500;  padding-right:10px;">
                                      <?php
                                      $chistatus= chidi('Air Condition');
                                      $dd = date("H:i:s");
 ...
 $(document).ready(function(){
  ...
  $('#insert_data').on('submit', function(event){
   ...
   var stat =  $('#hidden_ac').val();
   
//alert(stat);
   $confirm = confirm("Are you sure you want to switch "+stat+ " the ac? ");
   if($confirm == true)
   {
